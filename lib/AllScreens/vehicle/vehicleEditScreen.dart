...
class VehicleEditScreen extends StatefulWidget {
  static const String idScreen = "VehicleEditScreen";

  final String id,name,address,province,district,city,nearestTown,email,phone,dis,lat,lng,pic,pic1,category,seats;
  VehicleEditScreen({required this.id,required this.name,required this.address,required this.province,required this.district,required this.city,required this.nearestTown,required this.email,required this.phone,required this.dis,required this.lat,required this.lng,required this.pic,required this.pic1,required this.category,required this.seats}): super();

  @override
  _VehicleEditScreen createState() => _VehicleEditScreen();
}

class _VehicleEditScreen extends State<VehicleEditScreen> {
  ...
  initialise() {
    ...
    vehicleCategory=widget.category.toString();
    seatsTextEditingController.text=widget.seats.toString();
  }
  ...
  Widget build(BuildContext context) {
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      if (nameTextEditingController.text.length < 3) {
                        ...
                        displayToastMessage("Select Vehicle Category!", context);
                      } else if (seatsTextEditingController.text.isEmpty) {
                        displayToastMessage("No. of Seats Required!", context);
                      } else if (lat==""&&lng=="") {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text("Vehicle Registration Update"),
        ),
  ...
  void insertData(BuildContext context, String imageUrl, String imageUrl1) async {
    try {
   //   await firestore.collection("vehicles1").add({
      await firestore.collection("vehicles1").doc(widget.id.toString()).update({
        ...
        'seats': seatsTextEditingController.text,
